...
const table = new Tabulator("#movements-table", {
	...
	layout:"fitColumns",
	selectable:true,
 	ajaxURL:"movements_action.php?transaction=true",
	columns:[
		...
		{title:"Note", field:"note", hozAlign:"left", formatter:"textarea", editor:"input"}
		],
	...
	cellEdited:function(cell){
		...
		if(id){
			...
			myRequest.open("POST","movements_action.php");
			myRequest.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			myRequest.send("pk="+id+"&col="+col+"&value="+value+"&action=edit");
		}else{
			...
			if(cat_id && dat_mov && dat_mov != 'Invalid date' && usr_mov && val){
				...
				myRequest.open("POST","movements_action.php");
				myRequest.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
				myRequest.send("cat="+cat_id+"&value="+val+"&usr="+usr_mov+"&note="+note+"&date="+dat_mov+"&action=save");
			}else{
...
document.getElementById("delete").addEventListener("click", function(){
	...
	for (x in selectedRows) {
	    ...
	    myRequest.open("POST","movements_action.php");
	    myRequest.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	    myRequest.send("transaction=true&pk="+id+"&action=delete");
	}
	table.setData();
});
...
function getCategory(){
	myRequest = CreateXmlHttpReq(myHandler);
	myRequest.open("GET","movements_action.php?category=true", false);
	myRequest.send();
...
function getUser(){
	myRequest = CreateXmlHttpReq(myHandler);
	myRequest.open("GET","movements_action.php?user=true", false);
	myRequest.send();
	tempObj = JSON.parse(myRequest.responseText);
